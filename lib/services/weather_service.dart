...
class WeatherService {
      ...
      if (response.statusCode == 200) { 
        ...
        if (data['address'] != null) {
          ...
          if (cityName != null && cityName.isNotEmpty) { 
            debugPrint('Got city from Nominatim: $cityName'); 
            return cityName;
          } 
        }
      }
    } catch (e) { 
      debugPrint('Error getting city from Nominatim: $e');
    }
  ...
  Future<String> getCityName(double lat, double lon) async {
    ...
    try {
      ...
      if (placemarks.isNotEmpty) {
        ...
        // debug prints to help while developing
        debugPrint('Geocoding package results:');
      ...
      debugPrint('Error in placemarkFromCoordinates: $e');
    }
  ...
  try {
    ...
    // fallback location (Almaty)
    try {
      debugPrint('Using fallback city');

      const double lat = 43.2389;
      const double lon = 76.8897;

      final data = await getWeatherData(lat, lon);

      return WeatherData.fromJson(data, 'Almaty');
    } catch (e) {
      debugPrint('Fallback weather failed: $e');
  ...
  Future<List<CityWeather>> getNearbyCitiesWeather(double lat, double lon) async {
    ...
    for(var city in nearbyCities) {
      try {
        ...
        debugPrint('Error fetching weather for nearby city: $e');
      }
    }
    return results;
  }
  ...
  Future<List<DailyWeather>> getDailyForecast() async {
    try {
      ...
      debugPrint('Error getting daily forecast: $e');
      return [];
    }
  }
 
  String getWeatherIcon(String? icon) {
    // simple and naive mapping of icon names to asset names
    if (icon == null) return 'assets/sunny_cloud.png';

    // check for obvious keywords in a simple order
    if (icon.contains('clear')) return 'assets/sunny.png';
    if (icon.contains('night')) return 'assets/clear_night.png';
    if (icon.contains('partly') || icon.contains('cloud')) return 'assets/sunny_cloud.png';
    if (icon.contains('rain') || icon.contains('drizzle')) return 'assets/rain.png';
    if (icon.contains('thunder') || icon.contains('storm')) return 'assets/heavyrain_and_storm.png';
